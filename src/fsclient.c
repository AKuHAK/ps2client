...
int main(int argc, char **argv, char **env)
{
 ...
 if (network_validate_hostname(hostname, "18195", SOCK_STREAM) != 0 || (c < 0))
 {
#ifdef _WIN32
  network_cleanup();
#endif
...
int fsclient_ls(int sock, char *pkt, char *pathname)
{
 ...
 while (ps2netfs_dread(sock, pkt, dd, &dirent) > 0)
 {
  ...
  if (dirent.mode & 0x4000)
   fprintf_locked(stdout, 0, "l");
  else if (dirent.mode & 0x2000)
   fprintf_locked(stdout, 0, "-");
  else if (dirent.mode & 0x1000)
   fprintf_locked(stdout, 0, "d");
  else
   fprintf_locked(stdout, 0, "-");

  if (dirent.mode & 0x0100)
   fprintf_locked(stdout, 0, "r");
  else
   fprintf_locked(stdout, 0, "-");
  if (dirent.mode & 0x0080)
   fprintf_locked(stdout, 0, "w");
  else
   fprintf_locked(stdout, 0, "-");
  if (dirent.mode & 0x0040)
   fprintf_locked(stdout, 0, "x");
  else
   fprintf_locked(stdout, 0, "-");

  if (dirent.mode & 0x0020)
   fprintf_locked(stdout, 0, "r");
  else
   fprintf_locked(stdout, 0, "-");
  if (dirent.mode & 0x0010)
   fprintf_locked(stdout, 0, "w");
  else
   fprintf_locked(stdout, 0, "-");
  if (dirent.mode & 0x0008)
   fprintf_locked(stdout, 0, "x");
  else
   fprintf_locked(stdout, 0, "-");

  if (dirent.mode & 0x0004)
   fprintf_locked(stdout, 0, "r");
  else
   fprintf_locked(stdout, 0, "-");
  if (dirent.mode & 0x0002)
   fprintf_locked(stdout, 0, "w");
  else
   fprintf_locked(stdout, 0, "-");
  if (dirent.mode & 0x0001)
   fprintf_locked(stdout, 0, "x");
  else
   fprintf_locked(stdout, 0, "-");
