...
struct {
  char *pathname;
  DIR *dir;
} host_dd[10] = {{NULL, NULL}, {NULL, NULL}, {NULL, NULL}, {NULL, NULL},
...
int host_verify_request(char *pkt, int id, unsigned short len)
{
  ...
  if (((unsigned int)ntohl(host_hdr.number) != id)
      && (ntohs(host_hdr.length) != len)) {
...
int host_write(int sock, char *pkt)
{
  ...
  else reply.result = write(ntohl(request.fd), buffer, size);
...
int ule_ioctl_rename(int fd_dd, char *newpath)
{
  ...
  if (old_fd_dd == fd_dd) {
    ...
    if (old_req == HOST_OPEN_REQ) {
      ...
      if (result < 0) result = -4;// Error: Error from closing file
    }
